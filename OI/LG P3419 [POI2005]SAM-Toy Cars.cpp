...
int n, k, p, ans;
int Next[maxp], data[maxp];
bool vis[maxn];
std::queue <int> que[maxn];
struct cmp {
    inline bool operator() (const int& aa, const int& bb) {
        return Next[aa] < Next[bb];
    }
};
__gnu_pbds::priority_queue <int, cmp, pairing_heap_tag> nico;
...
int main ()
{
	...
	{
		que[data[i]].pop ();
		if (que[data[i]].empty()) Next[i] = p + 1;
		else Next[i] = que[data[i]].front();
	}
	for (rint i = 1; i <= p; i ++)
	{
		if (vis[data[i]]) {
