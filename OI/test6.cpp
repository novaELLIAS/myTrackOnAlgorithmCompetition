...
inline void writelen (long long x) {
	if (x<=0) {pt('0'), pt('\n'); return;}
	write(x); pt('\n');
}
...
inline void add (Re int x, Re long long val, Re int n) {
	while (x < n) dat[x] += val, x += lowbit(x);
}

inline long long que (Re int x) {
...
signed main ()
{
	register int n=read()+1, m=read(), opt;
	register long long x, y;
	for (register int i=1; i^n; ++i) add(i, read(), n);
	while (m --) {
		opt=read(), x=read(), y=read();
		if (opt^1) {
			if (x>y) swap(x, y);
			write(que(y)-que(x-1)); pt('\n');
		} else add (x, y, n);
