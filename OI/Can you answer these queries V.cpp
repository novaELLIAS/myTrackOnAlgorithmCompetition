...
int main() {
	...
	register int _, x1, y1, x2, y2, tmp;
	cin >> _;
	while (_ --) {
		//for (rint i = 0; i <= 40000; i ++) tr[i] = null;
		fastIO::cin >> n;
		build (1, 1, n);
		fastIO::cin >> m;
		while (m --) {
			cin >> x1 >> y1 >> x2 >> y2;
			if (y1<x2) {
				cout <<  maxx(query(root,x1,y1-1).rmax,0)+query(root,y1,x2).sum+maxx(query(root,x2+1,y2).lmax,0) << endl;
			} else {
                tmp = query (root,x2,y1).dat;
				tmp = maxx (tmp, query(root,x2,y1).lmax + query(root,x1,x2-1).rmax);
				tmp = maxx (tmp, query(root,y1+1,y2).lmax + query(root,x2,y1).rmax);
				tmp = maxx (tmp, query(root,x2,y1).sum + maxx(0, query(root,x1,x2-1).rmax) + maxx(0, query(root, y1 + 1,y2).lmax));
				cout << tmp << endl;
            }
		}
	}
